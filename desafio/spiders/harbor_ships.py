...
class HarborShips(scrapy.Spider):
    ...
    def parse_paranagua(self, response):
        table_titles = ["ATRACADOS", "PROGRAMADOS", "AO LARGO PARA REATRACAÇÃO", "AO LARGO", "ESPERADOS", "DESPACHADOS"]
        PORTO = 'Paranaguá'

        for title in table_titles:
            table_html = response.xpath(f'//table[.//*[text()="{title}"]]').extract_first()
            table_selector = scrapy.Selector(text=table_html)

            direction_column = 'count(//table/thead/tr/th[.=" Sentido"]/preceding-sibling::th)+1'
            expected_column = 'count(//table/thead/tr/th[.=" Previsto"]/preceding-sibling::th)+1'
            merchandise_column = 'count(//table/thead/tr/th[contains(text()," Mercadoria")]/preceding-sibling::th)+1'

            table_head_columns = len(table_selector.xpath(f'.//thead/tr[2]/th').extract())
            table_rows = table_selector.xpath(f'.//tbody/tr')
            for row in table_rows:
                ...
                direction = row.xpath(f'./td[{direction_column}-{SKIP_COLUMNS}]/text()').extract_first()
                expected_volume = row.xpath(f'./td[{expected_column}-{SKIP_COLUMNS}]/text()').extract_first()
                merchandise = row.xpath(f'./td[{merchandise_column}-{SKIP_COLUMNS}]/text()').extract_first()

                volume = expected_volume.split()
                is_in_tons = volume[1] == "Tons."

                product = Product(harbor=PORTO,
    ...
    def parse_santos(self, response):
        table_titles = ["LIQUIDO A GRANEL", "TRIGO", "GRANEIS DE ORIGEM VEGETAL", "GRANEIS SOLIDOS - IMPORTACAO", "GRANEIS SOLIDOS - EXPORTACAO", "ROLL-IN-ROLL-OFF", "LASH", "CABOTAGEM", "CONTEINERES", "PRIORIDADE C5", "SEM PRIORIDADE"]
        PORTO = 'Santos'
        ...
        for title in table_titles:
            ...
            for row in rows:
                direction = ', '.join(row.xpath(f'.//td[{direction_column}]/text()').extract())
                expected_volume = ', '.join(row.xpath(f'.//td[{expected_column}]/text()').extract())
                merchandise = ', '.join(row.xpath(f'.//td[{merchandise_column}]/text()').extract())
